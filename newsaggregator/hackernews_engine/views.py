...
class WordCountApi(APIView):
    ...
    def get(self,request):
        ...
        days_ago = datetime.datetime.now() - datetime.timedelta(days=day)
        less_days_ago = datetime.datetime.now() - datetime.timedelta(days=day-1)
        
        #datetime.date.today()#.filter(time__lte = F('time') - datetime.timedelta(days=day),time__gte = F('time') - datetime.timedelta(days=day-1))
